...
describe('withRetry', () => {
  ...
  it('debe lanzar el error después de agotar los reintentos', async () => {
    // Creamos el error fuera del mock para tener referencia a él
    const error = new Error('siempre falla');

    // Usamos mockImplementation — más control que mockRejectedValue
    // La función retorna una nueva Promise rechazada cada vez que se llama
    const operation = jest.fn().mockImplementation(() => Promise.reject(error));

    let caughtError: Error | null = null;

    // Construimos la promise capturando el error explícitamente
    const promise = withRetry(operation, {
      maxRetries: 2,
      baseDelayMs: 100,
    }).catch((err: Error) => {
  ...
  it('NO debe reintentar errores con retryable: false', async () => {
    ...
    expect(caughtError).not.toBeNull();
    expect(caughtError!.message).toBe('Email inválido');
    expect(operation).toHaveBeenCalledTimes(1);
  });
